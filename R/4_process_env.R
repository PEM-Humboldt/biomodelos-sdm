...
process_env.current <- function(clim.dataset, clim.dir, extension, crs.proyect, area.M,
  ...
  if (proj.models == "M-G" & compute.G == TRUE) env.Ras <- env_crop
  if (proj.models == "M-G" & compute.G == FALSE){
    env.Ras <- raster::stack(list.files(paste0(folder.sp, "/G_variables/Set_1/G/"), pattern = ".asc$", full.names = T, recursive = F))
  } 
  ...
  if(computeF == TRUE){
    ...
    for (i in 1:length(index_vars)) {
      ...
      if (!identical(other_F, character(0))) {
        ...
        if (projMod == "M_M") {
          complimentVars <- list.files(paste0(foldersp, "/M_variables/Set_1/"), pattern = ".asc", full.names = T, recursive = F)
        } else {
          complimentVars <- list.files(paste0(foldersp, "/G_variables/Set_1/G"), pattern = ".asc", full.names = T, recursive = F)
        }
        complimentVars <- complimentVars[-c(1:NclimCurrent)]
  
        for (a in 1:length(complimentVars)) {
          file.copy(
            from = complimentVars[a],
        ...
        for (a in 1:nlayers(env_F)) {
          raster::writeRaster(
            x = env_F[[a]],
            filename = paste0(
              foldersp, "/G_variables/Set_1/", info_cc, "/", names(env_F[[a]]), ".asc"
            ),
            overwrite = T, NAflag = -9999
          )
        }
