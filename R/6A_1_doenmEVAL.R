do.enmeval <- function(occ., bias.file, beta.mult, f.class, env.Mdir, env.Gdir, env.Fdir, do.future,
                       folder.sp, col.lon, col.lat, proj.models) {
  ...
  # competitor models
  eval1 <- ENMevaluate(
    occ = data., env = env.M, bg.coords = Sbg, method = "jackknife",
  ...
  dir.create(paste0(folder.sp, "/final_models_enmeval"))
  dir.create(paste0(folder.sp, "/final_models_enmeval/current"))
  
  for (i in 1:length(current_proj)) {
    writeRaster(current_proj[[i]], paste0(
...
do.enmeval2 <- function(occ., bias.file, beta.mult, f.class, env.Mdir, env.Gdir, env.Fdir, do.future,
                        folder.sp, col.lon, col.lat, proj.models, partitionMethod #jackknife or block
                        ) {
